...
contract BulletproofVerifier {
    ...
    InnerProductVerifier public ipVerifier;
    ...
    constructor (
        ...
        ipVerifier = InnerProductVerifier(_ipVerifier);
    ...
    returns (bool) {
        ...
        uint256 yPrecomputed = uint256(keccak256(
            commitments,
            multiRangeProof.aI.X,
            multiRangeProof.aI.Y,
            multiRangeProof.s.X,
            multiRangeProof.s.Y))
        .mod();
    ...
    function commit(alt_bn128.G1Point[m] ps, uint256[m] ss) internal view returns (alt_bn128.G1Point commitment) {
        commitment = ps[0].mul(ss[0]);
        for (uint256 i = 1; i < m; i++) {
            commitment = commitment.add(ps[i].mul(ss[i]));
        }
    }

    function powers(uint256 base) internal pure returns (uint256[m] pow) {
        pow[0] = 1;
        pow[1] = base;
        for (uint256 i = 2; i < m; i++) {
            pow[i] = pow[i-1].mul(base);
        }
    }

    function powers(uint256 base, uint256 firstPower, uint256 numberOfPowers) internal pure returns (uint256[] pow) {
        pow = new uint256[](numberOfPowers);
        pow[0] = 1;
        uint256 i = 0;
        if (firstPower != 0) {
            for (i = 0; i < firstPower; i++) {
                pow[0] = pow[0].mul(base);
            }
        } 
